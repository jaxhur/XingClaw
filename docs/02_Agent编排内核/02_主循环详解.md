Agnet主循环：`src/agent_core/agent_loop.py`

- 提问→思考→调工具→拿结果→再思考→..."的过程

## 主循环的核心流程

主循环就是一个 `while True`，每轮问 LLM 一次，如果 LLM 说"我要用工具"，就执行工具并继续循环；如果 LLM 说"我想好了"，就结束。
...
graph TD
    ...
    CheckSteering -->|"无"| CallLLM["调用 LLM 获取回复"]

    InjectSteering --> CallLLM
...
### 调用 LLM 获取回复 `_stream_assistant_response`
